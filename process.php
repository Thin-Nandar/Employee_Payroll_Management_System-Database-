...
function doInsert($jobid=0,$fileid=0) {
    if (isset($_POST['submit'])) {  
        ...
        if ($age < 20){
            ...
            $jobreg->APPLICANTID = $applicant_id; // Use the same applicant ID
            ...
            // Update the attachment file with the correct applicant ID
            $update_sql = "UPDATE `tblattachmentfile` 
                          SET `USERATTACHMENTID` = '{$applicant_id}' 
                          WHERE `FILEID` = '".date('Y').$fileid."'";
            $mydb->setQuery($update_sql);
            $mydb->executeQuery();

            $autonum->auto_update('APPLICANT');

            message("Your application has been submitted successfully. Please wait for company confirmation.","success");
...
function UploadImage($jobid=0){
    ...
    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
        message("File Not Supported. Please upload JPG, PNG, JPEG, or GIF files.","error");
        return "";
    } else {
